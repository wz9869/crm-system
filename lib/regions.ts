...
/**
 * Parse an address string and return the US state abbreviation.
 * Recognizes both abbreviations ("TX") and full names ("Texas").
 * Returns null if no US state is found.
 */
export function extractStateAbbr(address: string): string | null {
  ...
  if (abbrMatch) {
    for (let i = abbrMatch.length - 1; i >= 0; i--) {
      if (STATE_ABBR_SET.has(abbrMatch[i])) {
        return abbrMatch[i];
      }
    }
  }
...
/**
 * Build a sorted, deduplicated list of US state abbreviations
 * that actually appear in the given addresses.
 */
...
/** Get the full state name for display (e.g. "TX" → "Texas") */
